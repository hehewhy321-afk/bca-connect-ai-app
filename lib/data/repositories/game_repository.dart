...
class GameRepository {
  ...
  Future<GameStats> getStats(String gameId) async {
    ...
    if (jsonString == null) {
      return GameStats(gameId: gameId, highScore: 0, timesPlayed: 0);
    }
  ...
  Future<void> _updateStats(GameScore score) async {
    ...
    final newStats = GameStats(
      ...
      bestTimeMs:
          score.timeMs != null &&
              (stats.bestTimeMs == null || score.timeMs! < stats.bestTimeMs!)
          ? score.timeMs
          : stats.bestTimeMs,
    );
  ...
  Future<void> clearAllData() async {
    ...
    final keys = prefs.getKeys().where(
      (key) => key.startsWith(_scoresKey) || key.startsWith(_statsKey),
    );
