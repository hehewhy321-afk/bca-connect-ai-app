...
class CourseRepository {
  ...
  Future<List<Course>> getPublishedCourses({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (cached != null) {
        ...
        final courses = jsonList.map((e) => Course.fromJson(e)).toList();
        debugPrint(
          'Loaded ${courses.length} courses from cache (offline or error)',
        );
        return courses;
      }
    } catch (e) {
  ...
  Future<void> _fetchAndCacheCourseInBackground(
    String courseId,
    String cacheKey,
  ) async {
    try {
  ...
  Future<List<CourseChapter>> getCourseChapters(String courseId) async {
    ...
    try {
      ...
      if (cached != null) {
        ...
        final chapters = jsonList
            .map((e) => CourseChapter.fromJson(e))
            .toList();
        debugPrint(
          'Loaded ${chapters.length} chapters from cache for course $courseId',
        );
    ...
    if (isOnline) {
      try {
        ...
        debugPrint(
          'Fetched ${chapters.length} chapters from database for course $courseId',
        );
        return chapters;
      } catch (e) {
        debugPrint('Error fetching chapters: $e');
        throw Exception(
          'Unable to load course content. Please check your connection.',
        );
      }
    }
  ...
  Future<void> _fetchAndCacheChaptersInBackground(
    String courseId,
    String cacheKey,
  ) async {
    try {
  ...
  Future<CourseEnrollment?> getUserEnrollment(
    ...
    try {
      ...
      if (cached != null) {
        ...
        debugPrint(
          'Loaded enrollment from cache for user $userId, course $courseId',
        );
    ...
    if (isOnline) {
      try {
        ...
        await CacheService.set(
          ...
          duration:
              CacheKeys.shortCache, // Enrollment status changes more frequently
        );
        ...
        throw Exception(
          'Unable to check enrollment status. Please check your connection.',
        );
      }
    }
  ...
  Future<void> _fetchAndCacheEnrollmentInBackground(
    String userId,
    String courseId,
    String cacheKey,
  ) async {
    try {
      ...
      if (response != null) {
        ...
        debugPrint(
          'Background updated enrollment cache for user $userId, course $courseId',
        );
      }
    } catch (e) {
  ...
  Future<List<CourseEnrollment>> getUserEnrollments(String userId) async {
    ...
    try {
      ...
      if (cached != null) {
        final List<dynamic> jsonList = jsonDecode(cached);
        final enrollments = jsonList
            .map((e) => CourseEnrollment.fromJson(e))
            .toList();
        debugPrint(
          'Loaded ${enrollments.length} enrollments from cache for user $userId',
        );
    ...
    if (isOnline) {
      try {
        ...
        debugPrint(
          'Fetched ${enrollments.length} enrollments from database for user $userId',
        );
        return enrollments;
      } catch (e) {
        debugPrint('Error fetching enrollments: $e');
        throw Exception(
          'Unable to load enrollments. Please check your connection.',
        );
      }
    }
  ...
  Future<void> _fetchAndCacheEnrollmentsInBackground(
    String userId,
    String cacheKey,
  ) async {
    try {
  ...
  Future<void> enrollInCourse({
    ...
    try {
      ...
      final enrollmentCacheKey =
          '${CacheKeys.courses}_enrollment_${userId}_$courseId';
  ...
  Future<String> uploadPaymentScreenshot(String userId, String filePath) async {
    ...
    try {
      ...
      final fileName =
          'payment_${userId}_${DateTime.now().millisecondsSinceEpoch}.jpg';
      final storagePath = 'payments/$fileName';

      await _supabase.storage.from('resources').upload(storagePath, file);
